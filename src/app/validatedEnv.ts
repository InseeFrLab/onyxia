...
const assert: typeof _assert = process.env["NODE_ENV"] === "test" ? () => {} : _assert;
...
function getEnvVar(
    key: string,
    options: { mandatory: false; parseInt: true },
): number | undefined;
function getEnvVar(
    key: string,
    options?: { mandatory?: boolean; parseInt?: boolean },
...
export const getValidatedEnv = memoizee(() => ({
    ...
    "KUBERNETES": (() => {
        const KUB_SERVER_NAME = getEnvVar("KUB_SERVER_NAME", {
            "mandatory": false,
        });
    ...
    "MINIO": {
        ...
        "MINIMUM_DURATION": getEnvVar("MINIO_END_MINIMUM_DURATION_MS", {
            "parseInt": true,
        }),
    },
    "FOOTER": {
