...
const editableFieldKeys = ["gitName", "gitEmail", "githubPersonalAccessToken", "kaggleApiToken"] as const;
...
export const AccountIntegrationsTab = memo((props: Props) => {
    ...
    const getEvtFieldAction = useMemo(
        () => memoize(
            (_key: EditableFieldKey) => Evt.create<UnpackEvt<AccountFieldProps.EditableText["evtAction"]>>()
        ),
        []
    );

    const onStartEditFactory =
        useCallbackFactory(
            ([key]: [EditableFieldKey]) =>
                editableFieldKeys
    ...
    return (
        <div className={className}>
            ...
            {(["githubPersonalAccessToken", "kaggleApiToken"] as const).map(key => {
                ...
                const serviceName = (() => {
                    switch (key) {
                        case "githubPersonalAccessToken": return "GitHub";
                        case "kaggleApiToken": return "Kaggle";
                    }
                })();
                ...
                const envVarName = (() => {
                    switch (key) {
                        case "githubPersonalAccessToken": return "$GITHUB_TOKEN";
                        case "kaggleApiToken": return "$KAGGLE_TOKEN";
                    }
                })();
                ...
                return <AccountField
                    ...
                    helperText={<>
                        <Link
                            ...
                            target="__blank"
                        >
                            {t("link for token creation", { serviceName })}
                        </Link>
                        &nbsp;
                        {t("accessible as env")}
                        &nbsp;
                        <span className={classNames.envVar}>{envVarName}</span>
                    </>}
