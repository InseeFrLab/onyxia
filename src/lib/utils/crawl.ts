...
export function crawlFactory(
    ...
    async function crawlRec(params: { directoryPath: string; filePaths: string[]; }) {
        ...
        const { directoryBasenames, fileBasenames } = await list({ directoryPath });

        const toPath = (fileOrDirectoryBasename: string) => pathJoin(directoryPath, fileOrDirectoryBasename);
    ...
    async function crawl(params: { directoryPath: string }): Promise<{ filePaths: string[]; }> {
