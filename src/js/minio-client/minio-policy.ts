...
const createGetObjectPolicy = ({ Effect, Action, Principal }: any) => (
	resources: any
) => ({
	Effect,
...
const isStatementPoliciy = ({ Effect, Action, Principal }: any) => ({
	Effect: pEffect,
	Action: pAction,
...
export const createPolicyWithDirectory = (bucketName: string) => async (minioPath: string) => {
...
export const createPolicyWithoutDirectory = (bucketName: any) => async (
	minioPath: any
) => {
	const policiesString = await getBucketPolicy(bucketName);
	const policies = JSON.parse(policiesString);
