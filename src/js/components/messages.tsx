...
export const ServiceCreeMessage: FunctionComponent<{ id: string; message?: string; }> =
	({ id, message }) => (
...
export const ServiceEchecMessage: FunctionComponent<{ nom: string; }> =
	({ nom }) => (
...
export const ServiceDemarreMessage: FunctionComponent<{ elapsed: number, id: string }> =
	({ elapsed, id }) => (
...
export const ServiceSupprime: FunctionComponent<{ id: string; groupe?: boolean; }> =
	({ id, groupe = false }) => (
...
const Message: React.FunctionComponent<{
	...
	<div className="message-service">
		...
		<div className="message">
			{!id ? null : <div className={`nom ${color}`}>{id.split('/').slice(-1)[0]}</div>}
